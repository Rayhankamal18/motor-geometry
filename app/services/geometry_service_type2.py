...
from core.models_type2 import GeometryInputsType2
from core.optimizer_type2 import optimize_dc_type2
from core.progress import ProgressCallback


def geometry_type2_from_dict(data: Dict[str, Any]) -> GeometryInputsType2:
    required = (
        "L_bit",
        "L_S1",
        "L_BH",
...
def run_calculate_type2(payload: Dict[str, Any]) -> Dict[str, Any]:
    inp = geometry_type2_from_dict(payload)
    DC_A = float(payload["DC_A"])
    DC_B = float(payload["DC_B"])
    DC_C = float(payload["DC_C"])
    result = calc_bur_type2(inp, DC_A=DC_A, DC_B=DC_B, DC_C=DC_C)
    return asdict(result)


def run_optimize_type2(
    ...
    *,
    ...
    max_iter = int(payload.get("max_iter", 50000))
    ...
    if result.solutions:
        sols_sorted = sort_solutions(
            ...
            sort_mode,
            tie_breaker=lambda s: (s.DC_A, s.DC_B, s.DC_C),
        )
