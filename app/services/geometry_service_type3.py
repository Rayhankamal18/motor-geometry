...
from core.models_type3 import GeometryInputsType3
from core.optimizer_type3 import optimize_dc_type3
from core.progress import ProgressCallback


def geometry_type3_from_dict(data: Dict[str, Any]) -> GeometryInputsType3:
    required = (
        "L_bit",
        "L_S1",
        "L_BH",
    ...
    return GeometryInputsType3(
        ...
        Y_bent_sub_deg=float(data["Y_bent_sub_deg"]),
    )


def run_calculate_type3(payload: Dict[str, Any]) -> Dict[str, Any]:
    inp = geometry_type3_from_dict(payload)
    result = calc_bur_type3(
        inp,
...
def run_optimize_type3(
    ...
    *,
    ...
    max_iter = int(payload.get("max_iter", 35000))
    ...
    if result.solutions:
        sols_sorted = sort_solutions(
            ...
            sort_mode,
            tie_breaker=lambda s: (s.DC_A, s.DC_B, s.DC_C, s.DC_D),
        )
