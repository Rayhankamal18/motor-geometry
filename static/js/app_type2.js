(function () {
  ...
  async function showSolutionDetail(index) {
    ...
    try {
      const body = await API.calculateType2({
        ...geometry,
        DC_A: s.DC_A,
        DC_B: s.DC_B,
        DC_C: s.DC_C,
  ...
  function renderSolutions(solutions, geometry, statusText, statusType, meta) {
    ...
    if (thead && window.OptTooltips) {
      thead.innerHTML = OptTooltips.solutionsHeadRow(2, showBalance);
    }
    solutionsTable.innerHTML = solutions
      .map((s, i) => {
        const scoreCell =
  ...
  calcForm.addEventListener("submit", async (e) => {
    ...
    try {
      const body = await API.calculateType2({ ...readGeometry(), ...readFormFields(calcForm) });
      if (!body.ok) {
  ...
  optForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      const body = await API.optimizeType2Stream(
        {
          ...geometry,
