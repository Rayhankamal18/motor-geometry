(function () {
  ...
  const CALC_FIELDS = [
    ...
    ["E", "Panjang section E (ft)"],
    ["L1", "Panjang L1 = A+B (ft)"],
    ["L2", "Panjang L2 = C+D+E (ft)"],
    ["Bprime", "Sudut B′ (deg)"],
    ["B1", "Sudut B1 (deg)"],
    ["B2", "Sudut B2 (deg)"],
    ["Phi", "Sudut Φ (deg)"],
  ...
  async function showSolutionDetail(index) {
    ...
    try {
      const body = await API.calculateType4({
        ...geometry,
        DC_A: s.DC_A,
        DC_B: s.DC_B,
        DC_C: s.DC_C,
  ...
  function renderSolutions(solutions, geometry, statusText, statusType, meta) {
    ...
    if (thead && window.OptTooltips) {
      thead.innerHTML = OptTooltips.solutionsHeadRow(4, showBalance);
    }
  ...
  calcForm.addEventListener("submit", async (e) => {
    ...
    try {
      const body = await API.calculateType4({ ...readGeometry(), ...readFormFields(calcForm) });
      if (!body.ok) {
  ...
  optForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      const body = await API.optimizeType4Stream(
        {
          ...geometry,
